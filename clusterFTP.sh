...
function checkUtftpd(){
	...
	do
		...
		pross=`echo "$USERPASS" | ssh suporte@$line sudo -S ps ax | grep uftpd | grep -v grep`
		#echo "out: $pross"
		if [ -z "$pross" ]; then
...
function startUtftpd(){
	...
	do
		...
		echo "$USERPASS" | ssh suporte@$line sudo -S /etc/init.d/uftp stop
		#echo "$USERPASS" | ssh suporte@$line sudo -S uftpd -D $dirUFTP -B $uftpBytes

		if echo "$USERPASS" | ssh suporte@$line sudo -S ls $dirUFTP ; then
			echo "$USERPASS" | ssh suporte@$line sudo -S /usr/bin/uftpd -d -L /var/log/uftp.log -D $dirUFTP -t &
		else
			echo -e "Directory $dirUFTP don't exist... creating..."
			if echo "$USERPASS" | ssh suporte@$line sudo -S mkdir -p $dirUFTP ; then
				echo "$USERPASS" | ssh suporte@$line sudo -S chown -R suporte $dirUFTP
				echo "$USERPASS" | ssh suporte@$line sudo -S /usr/bin/uftpd -d -L /var/log/uftp.log -D $dirUFTP -t &
			else
...
function shutdownAllHosts(){
	...
	case $yn in
		[yY] ) while IFS= read -r line
				do
					echo "$line"
					echo "$USERPASS" | ssh suporte@$line sudo -S poweroff
				done < "$fileHosts"
...
echo -e "\nFinding file $1 in the $(ColorBlue $2) host:"
		if echo "$USERPASS" | ssh suporte@$2 sudo -S ls $1 ; then
...
function executeSudoCommandCluster(){
	...
	do
		...
		if echo "$USERPASS" | sshpass ssh suporte@$line sudo -S $command ; then
			let "ok+=1"
...
function executeSudoCommandClusterInAHost(){
	...
	read command
		...
		echo -e "\nExecuting command $command in the $(ColorBlue $line) host:"
		if echo "$USERPASS" | sshpass ssh suporte@$host sudo -S $command ; then
...
function executeCommandCluster(){
	...
	do
		echo -e "\nExecuting command $command in the $(ColorBlue $line) host:"
		if echo "$USERPASS" | sshpass ssh suporte@$line $command ; then
			let "ok+=1"
			echo -e "Command executed with $(ColorGreen success) - total hosts: $ok."
		else
			let "fail+=1"
